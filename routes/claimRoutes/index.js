...
const storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    cb(
      null,
      file.fieldname + counter.toString() + path.extname(file.originalname)
    );
  }
});
                ...
                (err, result) => {
                  if (err) {
                    ...
                    if (result.rows.length === 0) {
                      ...
                      if (damage === 1) {
                        refund =
                          (carPrice * 0.1).toString() +
                          "-" +
                          (carPrice * 0.2).toString();
                      } else if (damage === 2) {
                        refund =
                          (carPrice * 0.3).toString() +
                          "-" +
                          (carPrice * 0.4).toString();
                      } else if (damage === 3) {
                        refund =
                          (carPrice * 0.5).toString() +
                          "-" +
                          (carPrice * 0.6).toString();
                      }
